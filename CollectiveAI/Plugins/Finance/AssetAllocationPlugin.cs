...
{
    [KernelFunction]
    [Description("Optimize asset allocation across asset classes")]
        ...
        int horizon)
    {
        ...
        return
            $"Optimal allocation for {riskTolerance} ({horizon}yr): Stocks {allocation["Stocks"]}%, Bonds {allocation["Bonds"]}%, Alts {allocation["Alternatives"]}%";
    }

    [KernelFunction]
    [Description("Rebalancing recommendations based on current vs target allocation")]
    public string GetRebalancingRecommendations(
        [Description("Current allocation JSON")]
        string currentAllocation,
        [Description("Target allocation JSON")]
        string targetAllocation)
    {
        return
            "Rebalancing needed: Reduce Tech from 45% to 30% (-$3.2M), Increase Healthcare from 10% to 15% (+$1.1M), Add Energy position 5% (+$1.1M)";
    }

    [KernelFunction]
    [Description("Calculate correlation matrix for portfolio diversification")]
    public string CalculateCorrelationMatrix(
        [Description("List of assets")] string assets)
    {
        return
            $"Correlation Matrix for {assets}: Highest correlation: AAPL-MSFT (0.85), Lowest: TLT-SPY (-0.42), Portfolio correlation: 0.65 (reduce for better diversification)";
    }
}